		...
		if(id11.substring(0, id11.indexOf("(")).equalsIgnoreCase(id22.substring(0, id22.indexOf("(")))){
			try{
				...
				if(totalAmount > 4){
					...
					c.inventory.deleteItem(id2, c.inventory.findItemSlot(id2), 1);
					c.inventory.addItem(c.getItems().getItemId(item1), 1, -1);
					c.inventory.addItem(c.getItems().getItemId(item2), 1, -1);
				}else{
					amount1 = totalAmount;
					String item1 = id11.substring(0, id11.indexOf("(") + 1) + amount1 + ")";
					c.inventory.deleteItem(id, c.inventory.findItemSlot(id), 1);
	...
	public void mixPotion(int id, int id2){
		for(int j = 0; j < potMixing.length; j++){
			if((potMixing[j][0] == id && potMixing[j][1] == id) || (potMixing[j][1] == id && potMixing[j][0] == id2)){
				c.inventory.deleteItem(id, c.inventory.findItemSlot(id), 1);
				c.inventory.deleteItem(id2, c.inventory.findItemSlot(id2), 1);
				c.inventory.addItem(potMixing[j][2], 1, -1);
				c.inventory.addItem(potMixing[j][3], 1, -1);
				break;
			}
		}
	}
