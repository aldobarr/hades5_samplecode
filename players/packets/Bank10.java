...
public class Bank10 implements PacketType{
	public boolean removeIDS(int id){
		int ids[] = {3900, 3322, 5382, 3415, 6669, 1119, 1120, 1121, 1122, 1123, 1688};
		for(int id1 : ids)
			if(id == id1)
				return true;
		return false;
	...
	public void processPacket(Client c, int packetType, int packetSize){
		...
		switch(interfaceId){
			...
			case 3900:
				if(c.myShopId == 14)
					c.getShops().skillBuy(removeId, 5);
				else
			...
			case 5064:
				...
				c.bank.deposit(removeId, removeSlot, 10);
				break;
			...
			case 5382:
				...
				c.bank.withdraw(removeId, removeSlot, 10);
				break;
			...
			case 3322:
				if(c.duel == null || (c.duel != null && c.duel.status == 0)){
					...
					c.duel.stakeItem(c, removeId, removeSlot, 10);
				}
				break;
			...
			case 6669:
				if(c.duel != null && c.duel.status == 1)
					c.duel.fromDuel(c, removeId, removeSlot, 10);
				break;
			...
			case 1122:
			case 1123:
				c.getSmithing().readInput(c.playerLevel[c.playerSmithing], Integer.toString(removeId), c, 10);
				break;
		}
	}
}