...
public class Fletching{
	...
	public void makeArrows(int item1, int item2){
		for(int j = 0; j < arrows.length; j++){
			if((item1 == arrows[j][0] && item2 == arrows[j][1]) || (item2 == arrows[j][0] && item1 == arrows[j][1])){
				if(c.inventory.hasItem(item1, 15) && c.inventory.hasItem(item2, 15)){
					if(c.playerLevel[c.playerFletching] >= arrows[j][4]){
						c.inventory.deleteItem(item1, c.inventory.findItemSlot(item1), 15);
						c.inventory.deleteItem(item2, c.inventory.findItemSlot(item2), 15);
						c.inventory.addItem(arrows[j][2], 15, -1);
	...
	public void fletchBow(int index){
		...
		for(int j = 0; j < amount; j++){
			if(c.inventory.hasItem(logType[index], 1)){
				if(c.playerLevel[c.playerFletching] >= reqs[index] || fletchType == 3){
					c.inventory.deleteItem(logType[index], c.inventory.findItemSlot(logType[index]), 1);
					c.inventory.addItem(toAdd, amountToAdd, -1);
	...
	public void openFletching(int item){
		if(item == 1511){
			...
			c.getPA().sendText("What would you like to make?", 8879);
			...
			c.getPA().sendText("Shortbow", 8889);
			c.getPA().sendText("Longbow", 8893);
			c.getPA().sendText("Arrow Shafts", 8897);
			log = item;
		}else if(item == 1521){
			c.getPA().sendFrame164(8880);
			c.getPA().sendText("What would you like to make?", 8879);
			...
			c.getPA().sendFrame246(8885, 250, 52); // right pic
			c.getPA().sendText("Oak Shortbow", 8889);
			c.getPA().sendText("Oak Longbow", 8893);
			c.getPA().sendText("Arrow Shafts", 8897);
			log = item;
		}else if(item == 1519){
			c.getPA().sendFrame164(8880);
			...
			c.getPA().sendFrame246(8885, 250, 52); // right pic
			c.getPA().sendText("Willow Shortbow", 8889);
			c.getPA().sendText("Willow Longbow", 8893);
			c.getPA().sendText("Arrow Shafts", 8897);
			log = item;
		}else if(item == 1517){
			c.getPA().sendFrame164(8880);
			...
			c.getPA().sendFrame246(8885, 250, 52); // right pic
			c.getPA().sendText("Maple Shortbow", 8889);
			c.getPA().sendText("Maple Longbow", 8893);
			c.getPA().sendText("Arrow Shafts", 8897);
			log = item;
		}else if(item == 1515){
			c.getPA().sendFrame164(8880);
			...
			c.getPA().sendFrame246(8885, 250, 52); // right pic
			c.getPA().sendText("Yew Shortbow", 8889);
			c.getPA().sendText("Yew Longbow", 8893);
			c.getPA().sendText("Arrow Shafts", 8897);
			log = item;
		}else if(item == 1513){
			c.getPA().sendFrame164(8880);
			...
			c.getPA().sendFrame246(8885, 250, 52); // right pic
			c.getPA().sendText("Magic Shortbow", 8889);
			c.getPA().sendText("Magic Longbow", 8893);
			c.getPA().sendText("Arrow Shafts", 8897);
			log = item;
		}
		fletching = true;
