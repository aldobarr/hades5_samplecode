...
public class Herblore{
	...
	private Client c;
	private int[][] information = {{199, 249, 1, 3}, {201, 251, 5, 4}, {203, 253, 11, 5}, {205, 255, 20, 6}, {207, 257, 25, 8}, {3049, 2998, 30, 8}, {209, 259, 40, 9}, {211, 261, 48, 10}, {213, 263, 54, 11}, {3051, 3000, 59, 12}, {215, 265, 65, 13}, {2485, 2481, 67, 13}, {217, 267, 70, 14}, {219, 269, 75, 15}};
	private int[][] information2 = {{249, 121, 2428, 1, 25}, {251, 175, 2446, 5, 38}, {253, 115, 113, 12, 50}, {255, 127, 2430, 22, 63}, {257, 139, 2434, 38, 88}, {261, 2448, 2448, 40, 100}, {259, 145, 2436, 45, 100}, {263, 157, 2440, 55, 125}, {3000, 3026, 3024, 63, 143}, {265, 163, 2442, 66, 155}, {2481, 2454, 2452, 69, 158}, {267, 169, 2444, 72, 163}, {269, 189, 2450, 78, 175}, {2998, 6687, 6685, 81, 180}, {225, 15333, 15332, 96, 1000}};
	...
	private void idHerb(int slot){
		if(c.inventory.hasItem(information[slot][0])){
			if(c.playerLevel[c.playerHerblore] >= information[slot][2]){
				c.inventory.deleteItem(information[slot][0], c.inventory.findItemSlot(information[slot][0]), 1);
				c.inventory.addItem(information[slot][1], 1, -1);
	...
	public void superAntiFire(){
		if(c.playerLevel[c.playerHerblore] >= 85){
			c.inventory.deleteItem(2454, 1);
			c.inventory.deleteItem(4621, 1);
			c.inventory.addItem(15305, 1, -1);
			c.sendMessage("You make a " + c.getItems().getItemName(15305) + ".");
			c.getPA().addSkillXP(210 * Config.HERBLORE_EXPERIENCE, c.playerHerblore);
		}else
			c.sendMessage("You need a herblore level of 85 to make this pot.");
	}

	private void makePot(int herbId){
		if(c.inventory.hasItem(227) && c.inventory.hasItem(herbId)){
			int slot = getSlot(herbId);
			if(c.playerLevel[c.playerHerblore] >= information2[slot][3]){
				c.inventory.deleteItem(herbId, c.inventory.findItemSlot(herbId), 1);
				c.inventory.deleteItem(227, c.inventory.findItemSlot(227), 1);
				/*
				 * if (c.playerLevel[c.playerHerblore] < information2[slot][3] +
				 ...
				 * else {
				 */
				c.inventory.addItem(information2[slot][2], 1, -1);
