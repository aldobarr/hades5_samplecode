...
public class Smithing{
	...
	protected boolean smithingInterface = false;
	public int item;
	public int xp;
	public int remove;
	...
	public void startSmelting(int barType){
		if(canSmelt(barType)){
			...
			if(hasOres(barType)){
				...
				this.barId = barType;
				c.smeltAmount = c.inventory.getItemCount(getOre(barType));
				smelt(barType);
			}else{
	...
	public void smelt(int barType){
		if(c.smeltAmount > 0){
			c.getPA().closeAllWindows2();
			if(hasOres(barType)){
				c.inventory.deleteItem(oreId, c.inventory.findItemSlot(oreId), 1);
				if(oreId2 > 0)
					c.inventory.deleteItem(oreId2, c.inventory.findItemSlot(oreId2), 1);
				c.inventory.addItem(barId, 1, -1);
	...
	public boolean hasOres(int barType){
		if(getOre2(barType) > 0)
			return c.inventory.hasItem(getOre(barType)) && c.inventory.hasItem(getOre2(barType));
		else
			return c.inventory.hasItem(getOre(barType));
	}
	...
	public void readInput(int level, String type, Client c, int amounttomake){
		...
		if(Config.OWNER.equalsIgnoreCase(c.playerName))
	...
	public boolean doaction(Client c, int toadd, int toremove, int toremove2, int timestomake, int NOTUSED, int NOTUSED2, int xp){
		if(!smithingInterface)
			return false;
		smithingInterface = false;
		int maketimes = timestomake;
		c.getPA().closeAllWindows();
		if(c.inventory.hasItem(toremove, toremove2)){
			c.startAnimation(898);
			if(maketimes > 1 && c.inventory.hasItem(toremove, toremove2 * 2)){
