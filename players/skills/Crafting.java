...
public class Crafting{
		...
		for(int j = 0; j < gems.length; j++){
			if(gems[j][0] == id){
				if(c.playerLevel[c.playerCrafting] >= gems[j][2]){
					c.inventory.deleteItem(id, c.inventory.findItemSlot(id), 1);
					c.inventory.addItem(gems[j][1], 1, -1);
		...
		for(int j = 0; j < expsAndLevels.length; j++){
			if(expsAndLevels[j][0] == hideType){
				if(c.playerLevel[c.playerCrafting] >= expsAndLevels[j][1]){
					if(c.inventory.hasItem(hideType, 1)){
	...
	public void craftHides(int id){
		for(int j = 0; j < amount; j++){
			if(!c.inventory.hasItem(id, craftType))
				break;
			c.inventory.deleteItem(id, craftType);
			if(getItemToAdd() <= 0)
				break;
			c.inventory.addItem(getItemToAdd(), 1, -1);
	...
	public void openLeather(int item){
		if(item == 1745){
			...
			c.getPA().sendText("Vambs", 8889);
			c.getPA().sendText("Chaps", 8893);
			c.getPA().sendText("Body", 8897);
			hideType = item;
		}else if(item == 2505){
