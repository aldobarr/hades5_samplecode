...
public class Thieving{
		...
		for(int j = 0; j < npcThieving.length; j++){
			if(npcThieving[j][0] == id){
				if(c.playerLevel[c.playerThieving] >= npcThieving[j][1]){
					if(id == 1911 && Misc.random(c.playerLevel[c.playerThieving] + 2 - npcThieving[j][1]) != 1){
						c.getPA().addSkillXP(npcThieving[j][2] * Config.THIEVING_EXPERIENCE, c.playerThieving);
						c.inventory.addItem(4621, 1, -1);
						c.startAnimation(881);
						c.lastThieve = System.currentTimeMillis();
						c.sendMessage("You pluck a feather");
					}else if(id != 1911 && Misc.random(c.playerLevel[c.playerThieving] + 2 - npcThieving[j][1]) != 1){
						c.getPA().addSkillXP(npcThieving[j][2] * Config.THIEVING_EXPERIENCE, c.playerThieving);
						c.inventory.addItem(995, npcThieving[j][3], -1);
						c.startAnimation(881);
						c.lastThieve = System.currentTimeMillis();
						c.sendMessage("You thieve some money...");
						break;
					}else{
						c.handleHitMask(npcThieving[j][4]);
	...
	public void stealFromStall(int id, int amount, int xp, int level){
		...
		if(c.playerLevel[c.playerThieving] >= level){
			if(c.inventory.addItem(id, amount, -1)){
	...
	public int[][] npcThieving = {{1, 1, 8, 200, 1}, {18, 25, 26, 500, 1}, {9, 40, 47, 1000, 2}, {26, 55, 85, 1500, 3}, {20, 70, 152, 2000, 4}, {21, 80, 273, 3000, 5}, {1911, 75, 120, 1, 6}};
