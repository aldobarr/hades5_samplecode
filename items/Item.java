...
public class Item{
	...
	public static int amulets[] = {1654, 1656, 1658, 1660, 1662, 1664, 8081, 8033, 7968, 6585, 86, 87, 295, 421, 589, 1478, 1692, 1694, 1696, 1698, 1700, 1702, 1704, 1706, 1708, 1710, 1712, 1725, 1727, 1729, 1731, 4021, 4081, 4250, 4677, 6040, 6041, 6208, 1718, 1722, 6859, 6863, 6857};
	...
	private static String[] fullbody = {"witch", "warlock", "Chicken wings", "Royal shirt", "vanguard", "battle-mage", "trickster", "Christmas ghost", "Pernix", "Morrigan's leather body", "top", "shirt", "platebody", "plate", "Ahrims robetop", "Karils leathertop", "brassard", "Robe top", "robetop", "platebody (t)", "platebody (g)", "chestplate", "torso", "hauberk", "Dragon chainbody", "Statius' Platebody", "Vesta's Chaibody", "ele' blouse", "ele' blouse", "Prince tunic", "Princess blouse", "Elite void"};
	/* Fullhat covers your head but not your beard. */
	private static String[] fullhat = {"battle-mage", "med helm", "coif", "Dharok's helm", "hood", "Initiate helm", "Coif", "Helm of neitiznot", "Armadyl helmet", "Berserker helm", "Archer helm", "Farseer helm", "Warrior helm", "Void"};
	/* Fullmask covers your entire head. */
	private static String[] fullmask = {"cowl", "vanguard", "trickster", "Christmas ghost", "ganodermic visor", "full helm", "full", "mask", "Verac's helm", "Guthan's helm", "Karil's coif", "mask", "Torag's helm", "Void", "sallet"};

	private static String[] notFullBody = {"Leder"};
	private static String[] notFullHat = {"Robin"};
	...
	public static boolean isFullBody(int itemId){
		...
		for(int i = 0; i < fullbody.length; i++)
			if(weapon.toLowerCase().contains(fullbody[i].toLowerCase()) && !notFullBody(weapon))
				return true;
		return false;
	}

	public static boolean notFullBody(String weapon){
		for(String body : notFullBody)
			if(weapon.toLowerCase().contains(body.toLowerCase()))
				return true;
		return false;
	}

	public static boolean notFullHelm(String weapon){
		for(String hat : notFullHat)
			if(weapon.toLowerCase().contains(hat.toLowerCase()))
				return true;
		return false;
	}
	...
	public static boolean isFullHelm(int itemId){
		...
		for(int i = 0; i < fullhat.length; i++){
			if(weapon.toLowerCase().contains(fullhat[i].toLowerCase()) && !notFullHelm(weapon)){
				return true;
			}
		}
		return false;
	...
	public static boolean isFullMask(int itemId){
		...
		for(int i = 0; i < fullmask.length; i++){
			if(weapon.toLowerCase().contains(fullmask[i].toLowerCase())){
				return true;
			}
		}
		return false;
	...
	public static void loadData(){
		try(Scanner in = new Scanner(new File("./Data/data/stackable.cfg"))){
			while(in.hasNextLine()){
				String parts[] = in.nextLine().split("-");
				itemStackable[Integer.parseInt(parts[0].trim())] = Boolean.parseBoolean(parts[1].trim());
			}
		}catch(IOException e){
			System.out.println("Critical error while loading stackable data! Trace:");
			e.printStackTrace();
		}
		try(Scanner in = new Scanner(new File("./Data/data/notes.cfg"))){
			while(in.hasNextLine()){
				String parts[] = in.nextLine().split("-");
				itemIsNote[Integer.parseInt(parts[0].trim())] = Boolean.parseBoolean(parts[1].trim());
			}
		}catch(IOException e){
			System.out.println("Critical error while loading note data! Trace:");
			e.printStackTrace();
		}
		try(Scanner in = new Scanner(new File("./Data/data/equipment.cfg"))){
			while(in.hasNextLine()){
				String parts[] = in.nextLine().split("-");
				targetSlots[Integer.parseInt(parts[0].trim())] = Integer.parseInt(parts[1].trim());
			}
		}catch(IOException e){
